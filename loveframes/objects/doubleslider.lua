...
local path = string.sub(..., 1, string.len(...) - string.len(".objects.doubleslider"))
local loveframes = require(path .. ".libraries.common")

-- slider object
local newobject = loveframes.NewObject("doubleslider", "loveframes_object_doubleslider", true)
...
function newobject:initialize()
	...
	self.type = "doubleslider"
	self.text = "DoubleSlider"
	...
	self.internal = false
	self.internals = {}
	self.OnMinValueChanged	= nil
	self.OnMaxValueChanged	= nil
		
	self.OnRelease = nil
	
	-- create the slider button
	local minsliderbutton = loveframes.objects["doublesliderbutton"]:new(self, 'min')
	minsliderbutton.state = self.state
	table.insert(self.internals, minsliderbutton)
	
	local maxsliderbutton = loveframes.objects["doublesliderbutton"]:new(self, 'max')
	maxsliderbutton.state = self.state
	table.insert(self.internals, maxsliderbutton)
	
	-- set initial values
	self:SetMinValue(self.min)
	self:SetMaxValue(self.max)
...
function newobject:mousepressed(x, y, button)
	...
	-- это отключено - пока не ясно какой из хендлов двигать если щелкнули между ними
	-- если за пределами, то можно соответствующий
	-- если внутри, то видимо тот, который ближе
--[[	
...
--]]---------------------------------------------------------
function newobject:SetMinValue(value)
	...
	local buttonWidth, buttonHeight = self:GetButtonSize()
	
	-- move the slider button to the new position
	if slidetype == "horizontal" then
		local xpos = (width - 2 * buttonWidth) * ((newval - min) / (max - min))
...
--[[---------------------------------------------------------
	...
	- desc: sets the object's value
--]]---------------------------------------------------------
function newobject:SetMaxValue(value)
	...
	-- slider button object
	local sliderbutton = internals[2]
	local slidetype = self.slidetype
	local width = self.width
	local height = self.height
	local min = self.min
	...
	-- move the slider button to the new position
	if slidetype == "horizontal" then
		local xpos = buttonWidth + (width - 2 * buttonWidth) * ((newval - min) / (max - min))
		sliderbutton:MoveToX(xpos)
--[[		
	elseif slidetype == "vertical" then
		local ypos = height - height * ((newval - min) / (max - min))
...
--[[---------------------------------------------------------
	- func: GetMinValue()
	- desc: gets the object's min value
--]]---------------------------------------------------------
function newobject:GetMinValue()
...
--[[---------------------------------------------------------
	- func: GetMaxValue()
	- desc: gets the object's max value
--]]---------------------------------------------------------
function newobject:GetMaxValue()
...
function newobject:SetMax(max)
	...
	if self.maxValue > self.max then
		self.maxValue = self.max
	end
...
function newobject:SetMin(min)
	...
	if self.minValue < self.min then
		self.minValue = self.min
	end
...
function newobject:SetButtonSize(width, height)
	...
	local minsliderbutton = internals[1]
	local maxsliderbutton = internals[2]
	
	if minsliderbutton and  maxsliderbutton then
		minsliderbutton.width = width
		minsliderbutton.height = height
		
		maxsliderbutton.width = width
		maxsliderbutton.height = height
	end
